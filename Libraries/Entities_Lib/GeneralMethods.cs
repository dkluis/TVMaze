...
public static partial class GeneralMethods
{
    public static FindShowEpisodeResult FindShowEpisodeInfo(string program, string showEpisode)
    {
        ...
        var seInfo = MyRegExSeasonEpisode().Split(showEpisode);
        var seSeas = MyRegexSeason().Split(showEpisode);
    ...
    private static FindShowViaNameResult FindShowViaName(string program, string showName)
    {
        ...
        try
        {
            ...
            var shows = db.Shows.Where(
                                   s => (s.ShowName == showName || s.CleanedShowName == cleanedShowName || s.AltShowname == altShowName) && !tvmStatusList.Contains(s.TvmStatus) &&
            ...
            {
                shows = db.Shows.Where(s => (s.ShowName == showName || s.CleanedShowName == cleanedShowName || s.AltShowname == altShowName) && s.ShowStatus != "Skipping")
                          .ToList();
                ...
                } else
                {
                    ...
                    if (splitShowName.Length > 0)
                    {
                        altShowName = splitShowName[0];
                    }
                    shows = db.Shows.Where(s => (s.ShowName == showName || s.CleanedShowName == cleanedShowName || s.AltShowname == altShowName) && s.ShowStatus != "Skipping")
                              .ToList();
                    ...
                    } else
                    {
                        result.Found = false;
                        result.Message =
                                $"Found Including Skipping {inclSkipping} records, Found Excluding Skipping {exclSkipping} records, Found Alternate ShowName {alternateShowName} records";
                    }
                }
            }
    ...
    [GeneratedRegex("S[0-9]+E[0-9]+.", RegexOptions.IgnoreCase, "en-US")] private static partial Regex MyRegExSeasonEpisode();
    [GeneratedRegex("S[0-9]+.", RegexOptions.IgnoreCase, "en-US")] private static partial Regex MyRegexSeason();
}
